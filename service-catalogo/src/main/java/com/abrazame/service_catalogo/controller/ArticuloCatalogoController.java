...
public class ArticuloCatalogoController {
    ...
    /** PUT /catalogo/{id}/decrementar/{cantidad}  →  descontar stock N unidades (entrega a beneficiario)
     *  Postman: PUT http://localhost:9090/catalogo/1/decrementar/5
     *  Llamado automáticamente por service-gestion al confirmar una entrega. */
    @PutMapping("/{id}/decrementar/{cantidad}")
    public ArticuloCatalogo decrementarStockPut(@PathVariable Long id, @PathVariable int cantidad) {
        return service.decrementarStockCantidad(id, cantidad);
    }
