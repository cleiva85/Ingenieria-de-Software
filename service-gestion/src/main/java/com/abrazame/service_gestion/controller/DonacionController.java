...
public class DonacionController {
    ...
    @Autowired private WebClient catalogoWebClient;

    /** Suma stock en service-catalogo cuando una donación queda Aprobada. No bloquea si catálogo falla. */
    private void incrementarStockCatalogo(Donacion d) {
        if (d.getArticuloId() == null || d.getCantidad() == null) return;
        try {
            catalogoWebClient.put()
                    .uri("/catalogo/{id}/incrementar/{cantidad}", d.getArticuloId(), d.getCantidad())
                    .retrieve()
                    .toBodilessEntity()
                    .block();
        } catch (Exception e) {
            System.err.println("⚠ No se pudo incrementar stock en catálogo: " + e.getMessage());
        }
    }
