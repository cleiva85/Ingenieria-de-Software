...
public class DonacionService {
    ...
    public Donacion guardarDirecto(Donacion d) { return donacionRepository.save(d); }

    public Donacion cambiarEstadoConComentario(Long id, String nuevoEstado, String comentario) {
        Donacion d = buscarPorId(id);
        d.setEstado(nuevoEstado);
        if (comentario != null && !comentario.isBlank()) d.setComentarioVoluntario(comentario);
    ...
    if (datos.getRecibida() != null) d.setRecibida(datos.getRecibida());
    return donacionRepository.save(d);
  }
  ...
  public Map<String, Long> resumen() {
    ...
    /** Eliminar donación por ID — la baja se refleja en la BD */
    public void eliminar(Long id) {
        if (!donacionRepository.existsById(id))
            throw new RuntimeException("Donación no encontrada: " + id);
